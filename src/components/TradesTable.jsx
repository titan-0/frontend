        ...
        <table className="min-w-full text-sm table-fixed" style={{width: '100%'}}>
          ...
          <thead className="bg-gray-50 text-gray-700">
            ...
            <tr className="sticky top-[36px] z-0 bg-gray-50 border-t border-gray-200">
              <th className="px-3 py-2 text-left truncate">Trade ID</th>
              ...
              <th className="px-3 py-2 text-left truncate">Broker Order ID</th>
              <th className="px-3 py-2 text-left truncate">Broker Trade ID</th>
              <th className="px-3 py-2 text-left truncate">Internal Order ID</th>
              <th className="px-3 py-2 text-right truncate">Total Charges</th>
              <th className="px-3 py-2 text-right truncate">Brokerage</th>
              <th className="px-3 py-2 text-right truncate">GST</th>
              <th className="px-3 py-2 text-right truncate">STT</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {loading && rows.length === 0 ? (
              ...
              <tr>
                <td colSpan="19" className="px-3 py-10 text-center text-gray-600">
              ...
              rows.map((row) => (
                <tr key={row.trade_id} className="hover:bg-gray-50">
                  <td className="whitespace-nowrap px-3 py-2.5 truncate">{row.trade_id}</td>
                  ...
                  <td className="whitespace-nowrap px-3 py-2.5 text-xs truncate">{row.broker_order_id || '-'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-xs truncate">{row.broker_trade_id || '-'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-xs truncate">{row.internalorder_id || '-'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-right truncate">{row.total_charges?.toFixed(2) || '0.00'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-right truncate">{row.brokerage_charge?.toFixed(2) || '0.00'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-right truncate">{row.gst_charge?.toFixed(2) || '0.00'}</td>
                  <td className="whitespace-nowrap px-3 py-2.5 text-right truncate">{row.stt_charge?.toFixed(2) || '0.00'}</td>
                </tr>
              ))
            )}
          </tbody>
